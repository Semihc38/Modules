...
SELECT name AS country_name,(gnp*1000000)/population AS per_capita_gnp FROM country WHERE gnp !=0 AND population!=0 ORDER BY per_capita_gnp DESC;
...
SELECT population/surfacearea FROM country ORDER BY population/surfacearea DESC;
...
SELECT gnp-gnpold AS pre_and_curr_gnp,Abs(gnp-gnpold),name FROM country ORDER BY Abs(gnp-gnpold) ;
