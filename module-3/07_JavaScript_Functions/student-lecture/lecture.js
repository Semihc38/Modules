...
function scopeTest() {
  ...
  // it's content will be undefined, ut you can reference it
  ...
  {
    var thisIsVisibleEveryWhere=3
  }
}

function createSentenceFromUser(name, age, listOfQuirks = [], separator = ', ') {
  let description = `${name} is currently ${age} years old. Their quirks are: `;
  // join takes the elements in the array and made them a string with the separator between the elements
  return description + listOfQuirks.join(separator);
}

/**
 * Takes an array and, using array reduce with a anonymous functions, generates
 * their sum.
 * .reduce -return an array reduce/converted to a single value using an anonymous function
 * .reduce will gp though one element at a time passing an element to teh anonymous function
 ...
 * the reducer hold the value from the anonymous function each time though the loop
 *
 * @param {number[]} numbersToSum numbers to add up
 * @returns {number} sum of all the numbers
 */
function sumAllNumbers(numbersToSum) {
  //sum is the reducer which will hold the value for each iteration of the loop
  ...
  //a CallBackfn function is a function passed as a parameter to another function
 ...
 * .filter will return an array of elements that an anonymous-func said should be in the new array
 * .filter will go though the array one element at a time  passing element to anon-func
 * the anonymous function tells filter to add the element it was given to the new array
 * buy returning true when the anon-func received the element
 ...
 * return false if the element should not be included
